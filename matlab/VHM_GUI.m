function VHM_GUI
    ...
    UT_GUI.node_table_handle = uitable('Units','normalized'...
            ,'Position',[0.5 0.525 0.475 0.475]...
...
function show_timing(hObject,eventdata)
    global UT_GUI
    if(strcmp(get(UT_GUI.run_button_handle,'String'),'Run Model'))
        errordlg('Model not running','error','modal');
        return;
    end
    UT_GUI.time_display=1;
end
...
function display_log(hObject,eventdata)
    ...
    for i=1:UT_GUI.nx
        ...
        Heart_log.label_handle(i)=text('Units','data','Position',[500,1.5*(UT_GUI.nx-i)+0.75],'String',temp_string);
    ...
    switch hObject
        ...
        case UT_GUI.path_table_handle
            switch edited_column
                ...
                case {2,3}
                    if((eventdata.NewData>UT_GUI.nx)||(eventdata.NewData<1))
                        errordlg('Invalid data for this column','Invalid Entry','modal');
                        UT_GUI.path_table=get(hObject,'Data');
...
function save_model(hObject,eventdata)
    ...
    save(dir,'node_table','path_table','node_pos');
end
